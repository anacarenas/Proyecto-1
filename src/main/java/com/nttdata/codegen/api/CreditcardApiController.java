...
public class CreditcardApiController implements CreditcardApi {
    ...
    private final CreditcardApiDelegate delegate;

    public CreditcardApiController(@Autowired(required = false) CreditcardApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new CreditcardApiDelegate() {});
    }

    @Override
    public CreditcardApiDelegate getDelegate() {
        return delegate;
    }
